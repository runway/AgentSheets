...
2. **Write-time validation** checks structure (axis types on the right side,
   ...
   anything persists: a stale or hallucinated name is a check error before
   anything saves. `dry_run` runs the same checks without persisting.
...
7. **Every referenced variable or dimension must exist in the target scenario.** For a
   ...
   name resolves before anything persists (§6.1). A saved block acquires one
