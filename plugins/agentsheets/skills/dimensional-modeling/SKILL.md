...
(read them now), and detailed references you load with `skill_read` when the
...
3. Entries (variables and dimensions together) are the workspace's data
   ...
   **dimension mapping** — distinct from the value-axis mapping of axiom 11 —
   ...
   `inspect_dimension_mappings` (references/14-dimension-mappings.md).
...
## References (load by task with skill_read)
